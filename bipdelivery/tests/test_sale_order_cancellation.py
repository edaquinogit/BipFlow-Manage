...
class SaleOrderCancellationAPITest(TwoStoreFixtureMixin, TestCase):
    ...
    def test_cancelling_a_virtual_order_restocks_its_item(self) -> None:
        order = self._make_order(
            channel=SaleOrder.CHANNEL_VIRTUAL, quantities={self.product_b: 3}
        )
        self.product_b.refresh_from_db()
        self.assertEqual(self.product_b.stock_quantity, 7)

        response = self.client.patch(
            self._status_url(order), CANCEL_STATUS_PAYLOAD, format="json"
        )
    ...
    def test_cancelling_a_variant_order_restocks_the_selected_variant(self) -> None:
        ...
        self.assertEqual(movement.new_stock, 4)

    def test_cancelling_a_pdv_order_restocks_with_pdv_source(self) -> None:
        order = self._make_order(
            channel=SaleOrder.CHANNEL_LOJA_FISICA, quantities={self.product_b: 2}
        )
    ...
    def test_cancelling_an_order_whose_product_was_deleted_does_not_crash(self) -> None:
        order = self._make_order(
            channel=SaleOrder.CHANNEL_VIRTUAL, quantities={self.other_product_b: 1}
        )
    ...
    def test_viewer_role_cannot_cancel_an_order(self) -> None:
        order = self._make_order(
            channel=SaleOrder.CHANNEL_VIRTUAL, quantities={self.product_b: 1}
        )
        viewer = User.objects.create_user(
            username="cancel_viewer_b", password="testpass123"
        )
