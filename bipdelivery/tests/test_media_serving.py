...
def test_product_image_url_is_absolute(api_client, authenticated_user, test_category, mock_image):
    ...
    assert isinstance(image_url, str), f"Image URL should be string, got {type(image_url)}"
    assert image_url.startswith("http://") or image_url.startswith(
        "https://"
    ), f"Image URL should be absolute, got: {image_url}"
    assert "media/" in image_url, f"Image URL should contain 'media/' segment, got: {image_url}"
...
def test_product_without_image_returns_none(api_client, authenticated_user, test_category):
    ...
    image_url = get_response.data["image"]
    assert image_url is None, f"Image URL should be None when no image provided, got: {image_url}"
