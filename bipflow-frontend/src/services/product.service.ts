...
class ProductService {
  ...
  async getAll(): Promise<Product[]> {
    try {
      ...
      if (!validation.success) {
        console.error("❌ BipFlow: Registry Data Corruption Detected.", validation.error.format());
   ...
   */
  async update(id: number, productData: Partial<Product> | FormData): Promise<Product> {
    try {
      ...
      if (!validation.success) {
        console.warn(`⚠️ BipFlow: Schema mismatch on Asset ${id}.`, validation.error.format());
